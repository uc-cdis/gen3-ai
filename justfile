  ...
  for dir in services/*; do
    if [[ -n "${dir#services/}" ]]; then
      if [ "${dir#services/}" == "gen3_inference" ]; then
        ...
        LEGACY_MIGRATIONS_DIR="${dir}/db_migrations"
        if [ -d "$MIGRATIONS_DIR" ]; then
          print_header "just setup_db:" "running" "dbmate migrations for" "${dir#services/}" "..."
        ...
        elif [ -d "$LEGACY_MIGRATIONS_DIR" ]; then
          print_header "just setup_db:" "running" "legacy SQL migrations for" "${dir#services/}" "..."

          for migration_file in $(find "$LEGACY_MIGRATIONS_DIR" -name "*.sql" | sort -V); do
...
openapi:
  ...
  for dir in services/*; do
    if [ -f "${dir}/generate_openapi.py" ]; then
      ...
      "${dir}/.venv/bin/python" "${dir}/generate_openapi.py"
    fi
  done
  ...
  if command -v npx >/dev/null 2>&1; then
    npx -y @redocly/cli build-docs docs/autogenerated_openapi.json --output docs/api.html || \
      echo "Skipping docs/api.html build because redocly failed."
  else
    echo "Skipping docs/api.html build because npx is unavailable."
  fi
...
lint $SERVICE="all" $EXTRA_ARG="": _check_dependencies
  ...
  if [ $SERVICE == "all" ]; then
    ...
    start=$(date +%s.%N)
    uv run --directory "$SERVICE" ruff check ./src --config ../../ruff.toml --fix $EXTRA_ARG
    exit_code=$?
    end=$(date +%s.%N)
