...
async def authorize_request(
    ...
    Args:
        ...
        authz_service_name (str): The Arborist service to supply for the check.
    ...
    """
    authz_resources = authz_resources
    ...
    if not is_authorized:
        logging.debug(
            ...
            f"on `{authz_resources}` for service `{authz_service_name}`"
        )
        raise HTTPException(status_code=HTTP_403_FORBIDDEN)


async def get_user_id(token: HTTPAuthorizationCredentials | None = None, request: Request | None = None):
    """
...
async def get_username(token: HTTPAuthorizationCredentials | None = None, request: Request | None = None) -> str:
    """
