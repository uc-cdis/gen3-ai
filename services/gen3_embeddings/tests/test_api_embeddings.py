...
def test_bulk_read_unknown_collections(client, allow_authz):
    """Bulk read across unknown collections returns embeddings and their source collection metadata."""
    allow_authz("docs")
    ...
    create_resp = client.post(
        "/vectorstore/collections/docs/embeddings",
        json={"embeddings": [{"embedding": [1.0, 0.0, 0.0], "metadata": {}}]},
    )
    assert create_resp.status_code == 200
    embedding_id = create_resp.json()["embeddings"][0]["embedding_id"]

    bulk_resp = client.post("/embeddings/bulk", json=[embedding_id])
    assert bulk_resp.status_code == 200, bulk_resp.text
