...
URL_PREFIX = config("GEN3_INFERENCE_PROXY_URL_PREFIX", default="", cast=str)
...
# the x-forwarded-for chain in calls to the underlying INFERENCE server,
...
#          Additional authorization is applied on a per-INFERENCE Resource level within
#          this proxy service, these are a first gate for API-level access. See the
#          rest of the docs/service for more info on INFERENCE authz.
AUTHZ_SERVICE_NAME = config("GEN3_INFERENCE_PROXY_AUTHZ_SERVICE_NAME", default="gen3-inference", cast=str)
AUTHZ_SERVICE_RESOURCE = config(
    "GEN3_INFERENCE_PROXY_AUTHZ_SERVICE_NAME",
    default="/services/gen3-inference",
    cast=str,
)
