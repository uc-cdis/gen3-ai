...
def convert_chat_completion_stream_to_sse(
    ...
    async def _generator() -> AsyncGenerator[bytes]:
        ...
        Yields:
            bytes: SSE-encoded Open Responses event lines, ending with `data: [DONE]`.
        """
        usage_info = None
        sequence_number = 0
...
def chat_completion_to_openresponses_response(
    ...
    Args:
        ...
        metadata (dict[str, Any] | None): Metadata to set on the Open Responses response.
...
def _openai_chat_completion_to_openresponses(
    ...
    Returns:
        ResponseResource: The Open Responses equivalent of `openai_chat`.
    """
